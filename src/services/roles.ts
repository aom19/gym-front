...
export async function getRoles(params?: PaginationParams): Promise<PaginatedResult<Role>> {
    try {
        const { data } = await api.get<PaginatedResult<Role>>("/roles", { headers: getAuthHeaders(), params });
        return data;
    } catch (error) {
        handleError(error);
    }
